...
function pmpro_getLevelCost( &$level, $tags = true, $short = false ) {
	...
	// initial payment
	if ( ! $short ) {
		/* translators: %s: Formatted initial payment amount. */
	...
	if ( (float)$level->billing_amount > 0 ) {
		if ( $level->billing_limit > 1 ) {
			if ( $level->cycle_number == '1' ) {
				/* translators: 1: Formatted recurring amount, 2: Billing period (singular), 3: Number of additional payments, 4: Billing period label for the number of payments. */
				...
				/* translators: 1: Formatted recurring amount, 2: Billing cycle number, 3: Billing period label, 4: Number of additional payments. */
			...
			/* translators: 1: Formatted recurring amount, 2: Billing cycle number, 3: Billing period label. */
			...
			if ( $level->billing_amount === $level->initial_payment ) {
				if ( $level->cycle_number == '1' ) {
					if ( ! $short ) {
						/* translators: 1: Formatted recurring amount, 2: Billing period (singular). */
	...
	if ( $level->trial_limit ) {
		if ( (float)$level->trial_amount == 0 ) {
			...
			if ( $level->trial_limit == '1' ) {
				...
				/* translators: 1: Number of trial payments, 2: Formatted trial payment amount. */
	...
	if ( $tax_state && $tax_rate && ! pmpro_isLevelFree( $level ) ) {
		/* translators: 1: Tax state or region, 2: Tax percentage value. */
...
function pmpro_getLevelExpiration( &$level ) {
	...
	if ( $level->expiration_number ) {
		/* translators: 1: Membership expiration number, 2: Expiration period label. */
...
function pmpro_getLevelsExpiration( &$levels ) {
	...
	foreach ( $levels as $curlevel ) {
		if ( $curlevel->expiration_number ) {
			/* translators: 1: Membership level name, 2: Membership expiration number, 3: Expiration period label. */
		...
		if ( $lastpage < 7 + ( $adjacents * 2 ) ) {
			for ( $counter = 1; $counter <= $lastpage; $counter++ ) {
				if ( $counter == $page ) {
					...
					/* translators: %s: Page or step label. */
...
function pmpro_check_upload( $file_index ) {
	...
	if ( ! empty( $field) ) {
		...
		if ( ! empty( $allowed_mime_types ) && ! in_array( $filetype['ext'], $allowed_mime_types ) ) {
			/* translators: %s: Allowed file extensions list. */
